...
TOTAL_NUM_TO_DELETE=`db2 -x "select count(INSERTTIME) count from wscomusr.cacheivl where INSERTTIME < (current timestamp - 3 HOURS) with ur"`
...
db2 "delete from  wscomusr.cacheivl where INSERTTIME in (select  INSERTTIME from wscomusr.cacheivl where INSERTTIME < (current timestamp - 3 HOURS) fetch first 5000  rows only with ur)"
