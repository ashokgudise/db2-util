...
INST=`whoami`
typeset -u INST
LOGLOC=/data02/db2scripts/QBSCRIPTS/LOGS/CHECKLOG
ERRORFILE=${LOGLOC}/Practice_${DBNAME}_$RDATE.LOG
...
LOGFILE=${LOGLOC}/Practice1_${DBNAME}_$RDATE.LOG
...
TEAMMAIL="Jonathan.quicquaro@dcsg.com"
...
echo "         HOLIDAY READINESS REPORT FOR $DBNAME" >> $LOGFILE
echo " " >> $LOGFILE
...
echo "********************************************************************************" >> $LOGFILE
echo "SECTION A: DB2 MAINTENANCE" >> $LOGFILE
echo "********************************************************************************" >> $LOGFILE


echo "[ RUNSTATS ]" >> $LOGFILE
...
echo " " >> $LOGFILE
db2 "select substr(TBSP_NAME,1,24) as tbspname ,TBSP_TYPE, substr(TBSP_STATE,1,15) state, TBSP_TOTAL_SIZE_KB as tot_sz_KB,TBSP_UTILIZATION_PERCENT as util_pct,TBSP_LAST_RESIZE_TIME as last_resize_time from SYSIBMADM.TBSP_UTILIZATION order by TBSP_TYPE with ur" >> $LOGFILE
...
db2 "select substr(tbsp_name,1,14)ts_name,tbsp_total_pages as total_pages,tbsp_used_pages as used_pages,tbsp_page_top as high_water_mark,reclaimable_space_enabled from table (MON_GET_TABLESPACE (NULL,-1)) as ts where tbsp_name not like ('SYS%') and tbsp_type='DMS' with ur" >> $LOGFILE
