...
MAIL_ONLY="eCommDBA@dcsg.com"
MAIL_WITH_PAGER="dba@dcsg.com"
RDATE=`date +%m-%d-%y`
...
LOGLOC=/db2shared/db2scripts/QBSCRIPTS/LOGS/LOGUTLZ
...
find ${LOGLOC}  -name "Message*"  -type f  -mtime +$RPT_RETENTION  -exec rm -f {} \;
find ${LOGLOC}  -name "LogUtlz*"  -type f  -mtime +$RPT_RETENTION  -exec rm -f {} \;
...
db2 -tx  -r /db2shared/db2scripts/QBSCRIPTS/LOGS/LOGUTLZ/Attachment_${INST}_${DBNAME}_${DATE}.html<<@END_OF_STATEMENT
echo <HTML> ;
echo <HEAD> ;
echo <TITLE> ;
...
echo " " >> $MSG
echo " Associated Lock information"  >> $MSG
echo " ------------------------------------------------------------------ "  >> $MSG

v_applid=$(db2 -x select appl_id_oldest_xact from sysibmadm.snapdb)
ASSC_LOCK_INFO=$(db2 "call sysproc.am_get_lock_chns(${v_applid},?)")
echo "  ${ASSC_LOCK_INFO} "  >> $MSG

echo " " >> $MSG
echo " ------------------------------------------------------------------ "  >> $MSG

db2 "select substr(ai_h.appl_name,1,10) as Hold_App, substr(ai_h.primary_auth_id,1,10) as Holder, substr(ai_w.appl_name,1,10) as Wait_App, substr(ai_w.primary_auth_id,1,10) as Waiter, lw.lock_mode as Hold_Mode, lw.lock_object_type as Obj_Type, substr(lw.tabname,1,10) as TabName, substr(lw.tabschema,1,10) as Schema, timestampdiff(2,char(lw.snapshot_timestamp - lw.lock_wait_start_time)) as waiting_Secs from sysibmadm.snapappl_info ai_h, sysibmadm.snapappl_info ai_w, sysibmadm.snaplockwait lw where lw.agent_id = ai_w.agent_id and lw.agent_id_holding_lk = ai_h.agent_id with ur"  >> $MSG
...
mutt -s  "HIGH LOG UTILIZATION ALERT FOR $INST:$DBNAME ON $HOSTNM IN $ENVIRONMENT. THE CURRENT PERCENTAGE UTILIZATION IS $PERCENT_UTIL%" $MAIL_ONLY  < $MSG -a  /db2shared/db2scripts/QBSCRIPTS/LOGS/LOGUTLZ/Attachment_${INST}_${DBNAME}_${DATE}.html 


## cat $MSG; uuencode /db2shared/db2scripts/LOGS/LOGUTLZ/Attachment_${INST}_${DBNAME}_${DATE}.html  Log_Utilization_Rpt.html) |mail -s "HIGH LOG UTILIZATION ALERT FOR $INST:$DBNAME ON $HOSTNM. THE CURRENT PERCENTAGE UTILIZATION IS $PERCENT_UTIL%" $MAIL_ONLY
...
mutt -s  "HIGH LOG UTILIZATION ALERT FOR $INST:$DBNAME ON $HOSTNM IN $ENVIRONMENT. THE CURRENT PERCENTAGE UTILIZATION IS $PERCENT_UTIL%" $MAIL_WITH_PAGER  < $MSG -a  /db2shared/db2scripts/QBSCRIPTS/LOGS/LOGUTLZ/Attachment_${INST}_${DBNAME}_${DATE}.html

#(cat $MSG; uuencode /db2shared/db2scripts/LOGS/LOGUTLZ/Attachment_${INST}_${DBNAME}_${DATE}.html  Log_Utilization_Rpt.html) |mail -s "HIGH LOG UTILIZATION ALERT FOR $INST:$DBNAME ON $HOSTNM. THE CURRENT PERCENTAGE UTILIZATION IS $PERCENT_UTIL%" $MAIL_WITH_PAGER
