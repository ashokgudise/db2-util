...
DBNAME=$1
cd /db2shared/db2scripts/QBSCRIPTS/db2collect2_2022
...
echo "script.name="$C_SCRIPT_NAME               > collection.properties
echo "script.product="$C_SCRIPT_PRODUCT         >> collection.properties
echo "script.version="$C_SCRIPT_VERSION         >> collection.properties
echo "script.tz="$C_SCRIPT_TZ                   >> collection.properties
echo "script.start="$C_SCRIPT_START             >> collection.properties
echo "script.end="$C_SCRIPT_END                 >> collection.properties
echo "script.num_collections="$NUM_COLLECTIONS  >> collection.properties
echo "script.sleep="$SLEEP                      >> collection.properties
echo "script.user="$USER                        >> collection.properties
echo "script.db2.app_handle="$DB2_APP_HANDLE    >> collection.properties
echo "script.db2.db.name="$DBNAME               >> collection.properties
echo "script.db2.wcs.schema="$WCS_SCHEMA        >> collection.properties


FILELIST="$FILELIST collection.properties"
...
cp $THISSCRIPT mustgather.script

FILELIST="$FILELIST mustgather.script"
...
then
    ...
    echo "script.zipfile="$OUTFILE     >> collection.properties
    ...
    then
      ...
      echo "Unable to zip $OUTFILE $FILELIST db2collect.sh"
    fi

else

    OUTFILE=db2collect.$C_SCRIPT_END.tar.gz
    ...
    then
      ...
      echo "Unable to tar $OUTFILE $FILELIST db2collect.sh"
    fi
